...
# 4.6 反方质量与引文忠实性（v2 2026-08-22：数量之外查质量，08-21 审校沉淀）
if [ -f 08_洞察卡片.md ]; then
  HOLLOW=$(grep -nE '^\*\*反方\*\*[:：].{0,24}$' 08_洞察卡片.md | grep -cE '反方' )
  [ "$HOLLOW" -eq 0 ] 2>/dev/null || warn "反方疑似过短: $HOLLOW 行（详见 verify_quotes）"
fi
if [ -x "$(command -v python3)" ] && [ -f 01_清洗稿.md ]; then
  VQ_OUT=$(python3 "$SKILL_DIR/scripts/verify_quotes.py" "$DIR" 2>/dev/null)
  VQ_RC=$?
  if [ $VQ_RC -eq 0 ]; then
    ok "引文逐字+时间戳命中（$(echo "$VQ_OUT" | tail -1)）"
  else
    err "引文忠实性硬伤（verify_quotes）"
    echo "$VQ_OUT" | sed 's/^/    /'
  fi
fi
MUTI=$(grep -cE '^\*\*母题\*\*' 07_议题关联地图.md 2>/dev/null || echo 0)
[ "$MUTI" -ge 1 ] && ok "议题地图有母题" || warn "议题地图缺母题"
CODEBLOCKS=$(awk '/^```$/{c++; if(c>=2) exit} END{print c}' 07_议题关联地图.md 2>/dev/null || echo 0)
...
echo
echo -e "${B}[检查 8] 三层收敛（v2.3 B 规格）${N}"
EXEC="$DIR/00_执行摘要.md"
if [ -f "$EXEC" ]; then
  ...
  warn "缺 00_执行摘要.md——v2.3 起标准/重装档必写（10 秒简报层）"
fi
if [ -d "$DIR/02_主题整理" ]; then
  TOT=$(ls "$DIR"/02_主题整理/第*段_*.md 2>/dev/null | wc -l | tr -d ' ')
...
echo "=========================================="
echo
echo "── 收尾三问（经验回流，防 skill 与实战脱节）──"
echo "  1) 本场踩了新坑吗？→ 有则写入 skill 的 references/06-pitfalls.md"
echo "     （先 grep '^## P' 看最大编号再顺延，防撞号）"
echo "  2) 渲染 HTML 了吗？→ render_pack.py 末行的渲染质量自检必须 ✓"
echo "  3) 增量同步了吗？→ extract_atoms.py（递归）+ 索引 ✅ + 主题 README 链表"