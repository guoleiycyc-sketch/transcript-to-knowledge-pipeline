...
def load():
    p = os.path.join(ROOT, _CFG.get('paths', {}).get('atoms', '_kb/atoms.jsonl'))
...
RULES = [
    ...
    ('改装车', ['改装车']), ('宣讲', ['宣讲']), ('WorkBuddy 分档', ['WorkBuddy', '精英逻辑', '分档']),
...
def view_methods(atoms):
    ...
    lines = ['# 方法论总库 · 自动视图', '',
             f'> 由 atoms.jsonl 生成（{len(methods)} 条原子，归并 {len(merged)} 条）；出现场次 ≥3 = 宣讲课模块候选', '']
...
atoms = load()
gdir = os.path.join(ROOT, _CFG.get('paths', {}).get('views_dir', '_kb'))
