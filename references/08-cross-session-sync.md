...
**陷阱**：本 skill 把身份订正视为局部动作（头部映射改一下）。但实际上一处订正（如「X→F」「G→G」「H→H」）会扩散到 8 个位置。任何遗漏都会导致场次之间人物链条断裂、知识包互相打架。
...
**陷阱**：转写标签看起来是实名（「X」），实际是**某位参会者的腾讯会议账号名**，得到大脑按参会账号名标注转写。

**首次发现**：2026-08-19 客户丁场——F（F）使用腾讯会议账号「X」入会，docx 全部回合被转写标签为「X」。用户口头确认「X就是F」。

**检测**：

- 头部映射写「docx「X」218 回合」但本人从未自我介绍过此名
- 转写标签虽然有完整姓名形式，但与会者都未当面叫过此名（只叫「F/F/郭老师」）
- **跨场搜索**：在其他录音中查此名（如 `grep X <录音项目根>`）——无匹配 → 高概率是账号名
...
**陷阱**：很多场次有主源 + 交叉源（如腾讯会议全程 docx + A手机端得到大脑版；或腾讯会议全员版 + 单方手机录音）。时间戳偏移可达数分钟，错引就坐错。
...
| 组合 | 偏移 | 差异点 |
|---|---|---|
| 腾讯会议 docx + A手机端得到大脑 | ≈ 2 分钟（主源在前） | 主源有接通段/纪要元数据；交叉源有漏字 ASR |
...
```bash
# 1. 列表（按时间倒序）
笔记CLI notes --limit N -o json > /tmp/biji_recent.json
python3 -c "
import json
...
cfg = json.load(open(os.path.expanduser('~/.笔记CLI/config.json')))
